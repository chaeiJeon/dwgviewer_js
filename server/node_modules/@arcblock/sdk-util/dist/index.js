...
class BaseClient {
  constructor(config) {
    ...
    this.config = Object.assign({
      httpEndpoint: ds => "https://ocap.arcblock.io/api/".concat(ds),
      socketEndpoint: ds => "wss://ocap.arcblock.io/api/".concat(ds, "/socket"),
      enableQuery: true,
    ...
    if (!this.schema) {
      throw new Error("BaseClient: unsupported dataSource ".concat(this.config.dataSource));
    }
  ...
  doRawQuery(query) {
    ...
    let dataKey = arguments.length > 2 ? arguments[2] : undefined;

    try {
      const cleanQuery = print(parse(query));
      return this._doRequest(cleanQuery, requestOptions, dataKey);
    } catch (err) {
      throw new Error("BaseClient: invalid raw query ".concat(err.message || err.toString()));
    }
  }
  /**
  ...
  doBatchQuery(queries, requestOptions) {
    ...
    methods.forEach(x => {
      if (typeof this[x] !== 'function') {
        throw new Error("doBatchQuery got invalid query method ".concat(x));
      }

      if (this[x].type !== 'query') {
        throw new Error("doBatchQuery got none query method ".concat(x));
      }
    });
    ...
    documents.forEach(x => {
      variableDefinitions = variableDefinitions.concat(get(x, 'definitions[0].variableDefinitions', []));
  ...
  generateQueryFns() {
    ...
    Object.keys(builders).forEach(key => {
      const queryFn = async function queryFn(args) {
        let requestOptions = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
        const query = builders[key](_this._sanitizeArgs(args), (requestOptions || {}).ignoreFields);
        const result = await _this._doRequest(query, requestOptions, key);

        const pagedResult = _this._getPagedResults({
          result,
  ...
  generateMutationFns() {
    ...
    Object.keys(builders).forEach(key => {
      const mutationFn = async function mutationFn(args) {
        let requestOptions = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
        const query = builders[key](_this2._sanitizeArgs(args), (requestOptions || {}).ignoreFields);
        return _this2._doRequest(query, requestOptions, key);
      };
  ...
  async _doRequest(query, requestOptions, dataKey) {
    ...
    const httpEndpoint = typeof this.config.httpEndpoint === 'function' ? this.config.httpEndpoint(this.config.dataSource) : this.config.httpEndpoint; // combine custom headers and auth headers
    ...
    debug('_doRequest.headers', options.headers); // combine custom payload and graphql query

    const payload = Object.assign(await this._getExtraPayload(query), {
      query
    });
    ...
    if (res.status === 200) {
      ...
      if (Array.isArray(json.errors) && json.errors.length) {
        const err = new Error("GraphQLError: ".concat(json.errors.map(x => x.message).join('; ')));
    ...
    throw new Error("GraphQL Status Error ".concat(res.status));
  }
  /**
  ...
  async _ensureSubscriptionChannel() {
    ...
    const socketEndpoint = typeof this.config.socketEndpoint === 'function' ? this.config.socketEndpoint(this.config.dataSource) : this.config.socketEndpoint;

    const Socket = this._getSocketImplementation();

    this.socket = new Socket(socketEndpoint, this._getSocketOptions());
    this.socket.connect();
    this.socket.onMessage(_ref => {
      let {
        event,
        payload
      } = _ref;
      ...
      if (event === 'subscription:data') {
        const queryId = Object.keys(this.subscriptions).find(x => this.subscriptions[x].subscriptionId === payload.subscriptionId);

        if (queryId) {
          debug('subscription.onMessage', {
            queryId,
  ...
  _getPagedResults(_ref2) {
    ...
    keys.forEach(key => {
      ...
      const argPrefixStr = prefixStr.replace("".concat(dataKey), '').replace(/^\./, '');
  ...
  _attachNextFn(_ref3) {
    ...
    if (result.page && result.page.next && result.page.cursor && Array.isArray(result.data) && !result.next) {
      const pagingArgs = {
        paging: Object.assign({}, args.paging || {}, {
          cursor: result.page.cursor
        })
      };
      const newArgs = Object.assign({}, args, argPrefixStr ? {
        [argPrefixStr]: pagingArgs
      } : pagingArgs);
      debug('_attachNextFn', JSON.stringify({
        argPrefixStr,
        prefix,
        page: result.page,
        args,
